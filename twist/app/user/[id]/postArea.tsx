...
export default function PostArea({ token, userId }: PostAreaProps) {
    ...
    // Détermine l'URL en fonction de `userId`
    const { data, error, isLoading, mutate } = useSWR(
        token ? (userId ? `/api/profil/posts?userId=${userId}` : `/api/profil/posts`) : null,
        fetcher
    );
    ...
    const handleDeletePost = async (postId: number) => {
        if (userId) {
            alert("Tu ne peux pas supprimer les posts des autres !");
            return;
        }
            ...
            {data.posts.map((post: any) => (
                <div key={post.id} className="post-box">
                    <div className="post-header">
                        ...
                        {/* Affichage des options uniquement si c'est notre profil */}
                        {userId === undefined && (
                            <>
                                <span
                                    className="menu-icon"
